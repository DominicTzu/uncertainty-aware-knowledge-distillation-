...
OUT_DIR = "/root/exp/qwen25_3b_ukd_entropy_ultrachat"
...
# =========================
# Uncertainty-aware KD Trainer
# =========================
class UncertaintyAwareKDTrainer(Trainer):
    def __init__(
        self,
        teacher_model=None,
        kd_lambda=0.5,
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if active_student.numel() == 0:
            ...
            mean_entropy = torch.tensor(0.0, device=ce_loss.device)
            mean_weight = torch.tensor(0.0, device=ce_loss.device)
        else:
            # teacher/student distributions
            log_p_s = F.log_softmax(active_student / T, dim=-1)   # [N, V]
            p_s = log_p_s.exp()

            log_p_t = F.log_softmax(active_teacher / T, dim=-1)   # [N, V]
            p_t = log_p_t.exp()

            # entropy of teacher distribution: H(p_t) = -sum p log p
            entropy_t = -(p_t * log_p_t).sum(dim=-1)  # [N]

            # confidence weight: lower entropy -> larger weight
            weights = torch.exp(-self.uncertainty_alpha * entropy_t)  # [N]

            # token-wise KL: sum p_t * (log p_t - log p_s)
            token_kl = (p_t * (log_p_t - log_p_s)).sum(dim=-1)  # [N]

            # weighted average
            ukd_loss = (weights * token_kl).sum() / (weights.sum() + EPS)
            ukd_loss = ukd_loss * (T ** 2)

            mean_entropy = entropy_t.mean()
        ...
        if self.state.global_step % max(1, self.args.logging_steps) == 0:
            print(
                ...
                f"ukd={ukd_loss.item():.4f} "
                f"entropy={mean_entropy.item():.4f} "
                f"weight={mean_weight.item():.4f}"
            )
...
def main():
    ...
    trainer = UncertaintyAwareKDTrainer(
        model=student_model,
